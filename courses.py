...
PER_PAGE = 5
ALLOWABLE_PARAMS = ['name', 'category_id', 'short_desc', 'full_desc', 'author_id']
...
def review_params():
    return { param: request.form.get(param) for param in PARAMS_OF_REVIEW }
...
def course_reviews(course_id):
    ...
    sort_params = request.args.get('select_sort', 1, type=int)
    if sort_params == 1:
    ...
    search_parameters['select_sort'] = sort_params
